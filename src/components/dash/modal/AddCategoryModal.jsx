...
const AddCategoryModal = ({
  ...
  const { mutateAsync: createApi } = useMutation(
    async (data) => {
  ...
  const handleCategoryAdd = async () => {
    ...
    try {
      if (newCategory.trim() === "") {
        ...
        await createApi({ category_name: newCategory });
      }
    } catch (error) {
  ...
  return (
    <Transition appear show={addCategoryModalOpen} as={Fragment}>
                    ...
                    <div className="content">
                      ...
                      >
                        <h5 className="mb-4 text-22 text-textPrimary flex gap-3 items-center">
                          <Plus className="h-6 w-6" /> Add New Categoty
                        </h5>
                      </Dialog.Title>
